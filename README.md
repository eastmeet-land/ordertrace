...
2. Lua 필터 — `{앱}-{환경}-{로그타입}` 형태의 Elasticsearch 인덱스 자동 생성
...
**인덱스 네이밍 컨벤션**: `{앱}-{환경}-{로그타입}-YYYY.MM.DD`

| 환경 | 인덱스 | 내용 |
|------|--------|------|
| docker | `ordertrace-docker-app-2026.04.03` | API 요청/응답 로그 |
| docker | `ordertrace-docker-sql-2026.04.03` | SQL 실행 로그 |
| docker | `ordertrace-docker-error-2026.04.03` | 에러 로그 |
| dev | `ordertrace-dev-app-2026.04.03` | dev 환경 API 로그 |
| stg | `ordertrace-stg-error-2026.04.03` | stg 환경 에러 로그 |

Kibana Data View를 유연하게 구성하여 환경별, 로그 타입별 필터링이 가능합니다.

| Kibana Data View | Index Pattern | 용도 |
|-----------------|---------------|------|
| 전체 로그 | `ordertrace-*` | 모든 환경, 모든 타입 |
| docker 환경만 | `ordertrace-docker-*` | docker 환경 전체 |
| 에러만 (전 환경) | `ordertrace-*-error-*` | 모든 환경의 에러 |
| docker 에러만 | `ordertrace-docker-error-*` | docker 환경 에러 |
